...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{PGX_VERSIONCOMPARE_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{PGX_VERSIONCOMPARE_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require PGX::VersionCompare;

    PGX::VersionCompare->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
pgx_versioncompare_server.pl - Catalyst Testserver

=head1 SYNOPSIS

pgx_versioncompare_server.pl [options]
